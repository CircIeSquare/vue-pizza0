...
export default {
    data(){
        return {
            ...
            /*getMenuItems:{
                ...
                2:{
                    'name':"芝士pizza",
                    'discription':"芝士杀手，浓浓的芝士袜，食欲瞬间爆棚",
                    'options':[
                        {'size':9,'price':38},
                        {'size':12,'price':48}
                    ]
    ...
    computed:{
        getMenuItems(){
            //在vux中获取数据,通过$store.state
            //return this.$store.state.menuItems;

            //通过gettres获取数据
            return this.$store.getter.getMenuItems;
        },
        total(){
            let totalCost=0;
    ...
    methods:{
        fetchData(){
            ...
            //使用main.js中Vue.prototype.http=axios
